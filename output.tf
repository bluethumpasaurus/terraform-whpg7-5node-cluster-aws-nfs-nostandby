...
output "segment_server_3_private_ip" {
  description = "Private IP address of WHPG segment server 2 (server 4)."
  value       = aws_instance.server[3].private_ip
}

output "segment_server_4_private_ip" {
  description = "Private IP address of WHPG segment server 2 (server 5)."
  value       = aws_instance.server[4].private_ip
}
