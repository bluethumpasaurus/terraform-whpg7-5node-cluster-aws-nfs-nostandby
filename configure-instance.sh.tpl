...
dnf -y install whpg-backup
dnf install -y strace sysstat gdb lsof htop telnet sshpass nano tree wget nfs-utils
...
mkdir -p /mnt/backups
chown -R gpadmin:gpadmin /mnt/backups

# Create NFS SHARE 4 BACKUPS
cat >> /etc/fstab << 'EOF'
10.0.1.100:/srv/nfs/backups   /mnt/backups          nfs     defaults,_netdev  0      0
EOF
...
# NFS Server setup on server 1 - WarehousePG Coordinator (index 0)
if [ ${server_index} -eq 0 ]; then
  systemctl enable --now nfs-server
  systemctl enable --now rpcbind
  mkdir -p /srv/nfs/backups
  chown -R gpadmin:gpadmin /srv/nfs/backups
  chmod 770 /srv/nfs/backups

  # Configure the NFS Export
  cat > /etc/exports << EOC
/srv/nfs/backups   10.0.1.0/24(rw,sync,no_root_squash)
/srv/nfs/backups   10.0.2.0/24(rw,sync,no_root_squash)
EOC
...
# --- For hosts that are not '${server_index} -eq 0' (all others), configure the NFS Client ---
else
  echo "Running NFS Client setup on host ${server_index}..."
  systemctl daemon-reload
  mount /mnt/backups
...
# Install MinIO on server 2 - Segment Host 1 (index 1)
if [ ${server_index} -eq 1 ]; then
  echo "Installing MinIO on Segment Host 1"
